...
def start_server():
    ...
    if RUNNING_IN_DOCKER:
        ...
        uvicorn.run("bots.rebalancing_bot.main:app", host="0.0.0.0", port=9501, reload=True)
    else:
        print("⚠️  Not running inside Docker. Please run this command within a Docker container.")
        uvicorn.run("bots.rebalancing_bot.main:app", host="0.0.0.0", port=59003, reload=True)
        exit(1)
