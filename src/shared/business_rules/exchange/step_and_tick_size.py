# https://github.com/ccxt/ccxt/blob/master/python/ccxt/base/decimal_to_precision.py
...
def decimal_to_precision(n, rounding_mode=ROUND, precision=None, counting_mode=DECIMAL_PLACES, padding_mode=NO_PADDING):
    ...
    if precision is None:
        raise ValueError('precision should not be None')
    ...
    # Check if the precision is one of the allowed types (float, Decimal, or Int)
    if not isinstance(precision, (float, decimal.Decimal, numbers.Integral)):
        raise TypeError('precision has an invalid type: must be float, Decimal, or Integer')
    ...
    if counting_mode == TICK_SIZE:
        # TICK_SIZE logic requires a positive increment (e.g., 0.001)
        if not (precision > 0):
            raise ValueError('negative or zero precision can not be used with TICK_SIZE precisionMode')
    else:
        # DECIMAL_PLACES or SIGNIFICANT_DIGITS require a whole number (e.g., 8)
        if not isinstance(precision, numbers.Integral):
            raise TypeError('precision must be an integer (Integral) when not using TICK_SIZE')


    # assert rounding_mode in [TRUNCATE, ROUND]
    # assert counting_mode in [DECIMAL_PLACES, SIGNIFICANT_DIGITS, TICK_SIZE]
    # assert padding_mode in [NO_PADDING, PAD_WITH_ZERO]
    # Check Rounding Mode (0: TRUNCATE, 1: ROUND, etc.)
    if rounding_mode not in [TRUNCATE, ROUND]:
        raise ValueError(f'Invalid rounding_mode: {rounding_mode}. Must be TRUNCATE, ROUND, ROUND_UP, or ROUND_DOWN.')

    # Check Counting Mode (2: DECIMAL_PLACES, 3: SIGNIFICANT_DIGITS, 4: TICK_SIZE)
    if counting_mode not in [DECIMAL_PLACES, SIGNIFICANT_DIGITS, TICK_SIZE]:
        raise ValueError(
            f'Invalid counting_mode: {counting_mode}. Must be DECIMAL_PLACES, SIGNIFICANT_DIGITS, or TICK_SIZE.')

    # Check Padding Mode (5: NO_PADDING, 6: PAD_WITH_ZERO)
    if padding_mode not in [NO_PADDING, PAD_WITH_ZERO]:
        raise ValueError(f'Invalid padding_mode: {padding_mode}. Must be NO_PADDING or PAD_WITH_ZERO.')
